...
const POsApproved = ({route}) => {
    ...
    async function retrieveOrders() {
        try {
            const storedOrderList = MMKVwithEncryption.getString('ApprovedOrders');
    ...
    async function updateOrders() {
        try {
            const updatedOrderList = MMKVwithEncryption.getString('ApprovedOrders');
            ...
            <View style={{width: '100%', height: 140}}>
                ...
                >
                    <View style={[styles.orderBox, styles.standardBox]}>
                        <Pressable  style={{width: '100%', flex: 1, flexDirection: 'row',}}>
    ...
    const getOrdersByFilters = async (orderType, orderBranch, orderCompany ) => {
        try {
            ...
            await fetch('https://jdeps.nexovate.com:7077/jderest/v3/orchestrator/ORCH_NX_InquireApprovedOrders', {
                method: 'POST',
                headers: {
                    'jde-AIS-Auth': token,
                    'Content-Type':'application/json',
...
POsApproved.options = ({navigation}) => {
    return({
        headerStyle: {
